...
module.exports = function (src, replace, async){
  ...
  var REQUIRERE = async
    ? /^require(?:\s*\/\*[\s\S]*?\*\/\s*)?(?:(?:\.(?:\s*\/\*[\s\S]*?\*\/\s*)?[a-zA-Z_$][\w$]*)|(?:\[(?:\s*\/\*[\s\S]*?\*\/\s*)?(['"]).*?\1(?:\s*\/\*[\s\S]*?\*\/\s*)?\]))?(?:\s*\/\*[\s\S]*?\*\/\s*)?\((?:\s*\/\*[\s\S]*?\*\/\s*)?(?:['"]|\[)/
    : /^require(?:\s*\/\*[\s\S]*?\*\/\s*)?\((?:\s*\/\*[\s\S]*?\*\/\s*)?['"]/;
  var FLAGRE = /^require(?:\s*\/\*[\s\S]*?\*\/\s*)?(?:(?:\.(?:\s*\/\*[\s\S]*?\*\/\s*)?([a-zA-Z_$][\w$]*))|(?:\[(?:\s*\/\*[\s\S]*?\*\/\s*)?(['"])(.*?)\2(?:\s*\/\*[\s\S]*?\*\/\s*)?\]))/;
  var CHAINRE = /^[\w$]+(?:\s*\.\s*[\w$]+)*/;
